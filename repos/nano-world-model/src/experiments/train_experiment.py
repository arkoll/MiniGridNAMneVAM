...
class NanoWMTrainingModule(LightningModule):
    def __init__(self, args):
        ...
        self.joint_action_enabled = bool(joint_cfg.get("enabled", False))
        ...
        if self.joint_action_enabled:
            ...
            self.model.capture_hidden_features = True
            self.action_head = DiscreteActionHead(
                input_dim=self.model.hidden_size,
                model_dim=int(joint_cfg.get("hidden_size", 256)),
                num_actions=int(joint_cfg.get("num_classes", 5)),
                num_transitions=int(joint_cfg.get("num_transitions", 3)),
                num_layers=int(joint_cfg.get("num_layers", 2)),
                num_heads=int(joint_cfg.get("num_heads", 4)),
        ...
        if self.action_head is not None:
            trainable_parameters.extend(self.action_head.parameters())
    ...
    def _compute_action_metrics(self, batch, clean_context_latents):
        ...
        if raw_targets.shape[1] < num_transitions:
            raise ValueError(
                f"Need {num_transitions} action labels, got {raw_targets.shape[1]}"
            )
        raw_targets = raw_targets[:, :num_transitions]
        if raw_targets.ndim == 3:
            targets = raw_targets.argmax(dim=-1)
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step < self.args.experiment.training.gradient_clip_start_step:
            ...
            gradient_norm = clip_grad_norm_(self.opt.param_groups[0]["params"], self.args.experiment.training.gradient_clip_norm, clip_grad=True)
...
class TrainExperiment(BaseExperiment):
    ...
    def training(self):
        ...
        trainer_precision = args.experiment.infra.get("precision", None)
        if trainer_precision is None:
            trainer_precision = "bf16-mixed" if args.experiment.infra.mixed_precision else "32"
        trainer = Trainer(
            accelerator="gpu",
            devices=num_gpus,
