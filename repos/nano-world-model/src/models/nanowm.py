...
class NanoWM(nn.Module):
    ...
    def __init__(
        ...
        self.action_injection_type = action_injection_type
        self.causal = causal
        # Optional readout point for auxiliary heads. It is disabled by
        # default so upstream NanoWM behavior and memory use are unchanged.
        self.capture_hidden_features = False
    ...
    def forward(self, 
        ...
        # Preserve the final spatiotemporal transformer representation before
        # the diffusion output projection. The Baba joint model reads this
        # tensor to classify the three discrete transitions while the video
        # backbone itself remains action-unconditioned.
        if self.capture_hidden_features:
            self.last_hidden_features = rearrange(
                x, '(b f) p d -> b f p d', b=batches
            )
