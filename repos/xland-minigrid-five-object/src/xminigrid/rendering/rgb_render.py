...
def _render_diamond(img: np.ndarray, color: int):
    _render_floor(img, Colors.BLACK)

    def point_in_diamond(x: float, y: float) -> bool:
        return abs(x - 0.5) + abs(y - 0.5) <= 0.36

    fill_coords(img, point_in_diamond, COLORS_MAP[color])
